...
async function main() {
    ...
    const maxx = x[1]
    const maxy = y[1]+1
    const grid = Array.from(Array(maxy + 1), () => new Array(COLUMNS).fill('.'))
    grid.push(new Array(COLUMNS).fill('#'))
    
    const MOVE_OVER=COLUMNS/2-parseInt((maxx-minx)/2)
    ...
    //simulate sand fall
    let count = 0
    while (!sandPour(grid, 500+MOVE_OVER, minx)) {
        count++
    }
    printGrid(grid)
    console.log(count);
...
function sandPour(grid, col, minx, r = 0) {
    col -= minx
    if(grid[0][col] ==='o'){console.log('filled'); return true}
